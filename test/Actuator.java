...
/*@
predicate_ctor Actuator_shared_state(Actuator a, int min, int max) () =
	a.value |-> ?v &*&
	min <= v &*& v <= max;

predicate ActuatorInv(Actuator s;) =
	s.mon |-> ?l &*&
	s.name |-> ?n &*&
	s.min |-> ?mi &*& 
	s.max |-> ?ma &*&
    	l != null &*& 
	lck(l, 1, Actuator_shared_state(s, mi, ma)) &*&
	mi <= ma;
@*/
...
class Actuator {
	...
	Actuator(String name, int min, int max)
	//@ requires min <= max;
